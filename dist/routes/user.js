 ...
 * Block a user
 */
router.post('/me/block/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const currentUserId = req.user.id;
    ...
    `).catch(() => {
        ...
        const { rows: userRows } = await db_1.pool.query(`SELECT id FROM users WHERE id = $1 AND is_active = TRUE`, [blockedUserId]);
        if (userRows.length === 0) {
            return res.status(404).json({ message: 'User not found' });
        }
        // Block user (insert or update)
        const { reason } = req.body;
        await db_1.pool.query(`INSERT INTO blocked_users (blocker_id, blocked_id, reason)
       VALUES ($1, $2, $3)
       ON CONFLICT (blocker_id, blocked_id) 
       DO UPDATE SET reason = EXCLUDED.reason, created_at = NOW()`, [currentUserId, blockedUserId, reason || null]);
        return res.json({
            success: true,
 ...
 * Unblock a user
 */
router.delete('/me/block/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const currentUserId = req.user.id;
        const blockedUserId = req.params.userId;
        await db_1.pool.query(`DELETE FROM blocked_users 
       WHERE blocker_id = $1 AND blocked_id = $2`, [currentUserId, blockedUserId]);
        return res.json({
            success: true,
 ...
 */
router.get('/me/blocked', auth_1.authenticateToken, async (req, res) => {
    try {
        const currentUserId = req.user.id;
        const { rows } = await db_1.pool.query(`SELECT 
        bu.blocked_id as id,
       ...
       ORDER BY bu.created_at DESC`, [currentUserId]);
 ...
 * Report a user
 */
router.post('/me/report/:userId', auth_1.authenticateToken, async (req, res) => {
    try {
        const currentUserId = req.user.id;
    ...
    `).catch(() => {
        ...
        // Check if user exists
        const { rows: userRows } = await db_1.pool.query(`SELECT id FROM users WHERE id = $1 AND is_active = TRUE`, [reportedUserId]);
        if (userRows.length === 0) {
            return res.status(404).json({ message: 'User not found' });
        }
        ...
        await db_1.pool.query(`INSERT INTO user_reports (reporter_id, reported_id, reasons, additional_info)
       VALUES ($1, $2, $3, $4)
       ...
       DO UPDATE SET 
         ...
         created_at = NOW(),
         status = 'pending'`, [currentUserId, reportedUserId, reasons, additionalInfo || null]);
      ...
      `).catch(() => {
            ...
            await db_1.pool.query(`INSERT INTO blocked_users (blocker_id, blocked_id, reason)
         VALUES ($1, $2, $3)
         ON CONFLICT (blocker_id, blocked_id) DO NOTHING`, [currentUserId, reportedUserId, 'Blocked after report']);
        }
        return res.json({
            success: true,
