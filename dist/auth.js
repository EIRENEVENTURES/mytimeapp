...
router.get('/validate/username/:username', async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query('SELECT id FROM users WHERE username = $1', [
...
router.get('/validate/phone/:phone', async (req, res) => {
    ...
    try {
        const { rows } = await db_1.pool.query('SELECT id FROM users WHERE phone_number = $1', [
            phone.trim(),
        ]);
        if (rows.length > 0) {
            return res.json({
...
router.post('/otp/send', async (req, res) => {
    ...
    try {
        ...
        // Insert new OTP
        await client.query(`INSERT INTO otp_verifications (email, otp_code, expires_at)
       VALUES ($1, $2, $3)`, [email.toLowerCase().trim(), otpCode, expiresAt]);
...
router.post('/otp/verify', async (req, res) => {
    ...
    const client = await db_1.pool.connect();
    try {
        const { rows } = await client.query(`SELECT id, expires_at, verified
       FROM otp_verifications
       WHERE email = $1 AND otp_code = $2
       ORDER BY created_at DESC
       LIMIT 1`, [email.toLowerCase().trim(), otpCode]);
        if (rows.length === 0) {
...
router.post('/signup', async (req, res) => {
    const { email, password, displayName, username, phoneNumber, otpCode, country } = req.body ?? {};
    ...
    try {
        ...
        // Verify OTP first
        const otpResult = await client.query(`SELECT id, expires_at, verified
       FROM otp_verifications
       WHERE email = $1 AND otp_code = $2
       ORDER BY created_at DESC
       LIMIT 1`, [String(email).toLowerCase().trim(), otpCode]);
        ...
        const { rows } = await client.query(`INSERT INTO users (email, password_hash, display_name, username, phone_number, email_verified, country)
       ...
       RETURNING id, email, display_name, username, phone_number, role, country`, [
        ...
        const { accessToken, refreshToken, refreshExpiresAt } = generateTokens(user.id, user.email, user.role);
        await client.query(`INSERT INTO sessions (user_id, refresh_token, user_agent, ip_address, expires_at)
       VALUES ($1, $2, $3, $4, $5)`, [user.id, refreshToken, req.headers['user-agent'] ?? null, req.ip, refreshExpiresAt]);
...
router.post('/login', async (req, res) => {
    ...
    const client = await db_1.pool.connect();
    try {
        const { rows } = await client.query(`SELECT id, email, password_hash, display_name, role, username, phone_number
       FROM users
       WHERE is_active = TRUE
        ...
        const valid = await bcrypt_1.default.compare(String(password), user.password_hash);
        if (!valid) {
            return res.status(401).json({ message: 'Invalid credentials' });
        }
        const { accessToken, refreshToken, refreshExpiresAt } = generateTokens(user.id, user.email, user.role);
...
router.post('/refresh', async (req, res) => {
    ...
    try {
        const now = new Date();
        const { rows } = await client.query(`SELECT s.id, s.user_id, s.expires_at, s.revoked_at,
       ...
       WHERE s.refresh_token = $1`, [refreshToken]);
        ...
        const { accessToken, refreshToken: newRefresh, refreshExpiresAt } = generateTokens(row.user_id, row.email, row.role);
        await client.query(`UPDATE sessions SET revoked_at = NOW() WHERE id = $1`, [row.id]);
        await client.query(`INSERT INTO sessions (user_id, refresh_token, user_agent, ip_address, expires_at)
       VALUES ($1, $2, $3, $4, $5)`, [row.user_id, newRefresh, req.headers['user-agent'] ?? null, req.ip, refreshExpiresAt]);
        return res.json({
            user: {
                id: row.user_id,
                email: row.email,
...
router.post('/reset-password/send-otp', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const { rows } = await client.query('SELECT email, password_hash, display_name FROM users WHERE id = $1', [userId]);
        if (rows.length === 0) {
            await client.query('ROLLBACK');
        ...
        const valid = await bcrypt_1.default.compare(String(currentPassword), user.password_hash);
        if (!valid) {
            await client.query('ROLLBACK');
            return res.status(401).json({ message: 'Current password is incorrect' });
        }
        // Ensure new password is different from old
        const isSame = await bcrypt_1.default.compare(String(newPassword), user.password_hash);
        if (isSame) {
            await client.query('ROLLBACK');
            return res
                .status(400)
        ...
        await client.query(`INSERT INTO otp_verifications (email, otp_code, expires_at)
       VALUES ($1, $2, $3)`, [user.email.toLowerCase().trim(), otpCode, expiresAt]);
        const emailHtml = `
<!DOCTYPE html>
<html>
<head>
...
router.post('/reset-password/confirm', auth_1.authenticateToken, async (req, res) => {
    ...
    try {
        await client.query('BEGIN');
        const { rows: userRows } = await client.query('SELECT email, password_hash, display_name FROM users WHERE id = $1', [userId]);
        ...
        // Verify OTP
        const { rows: otpRows } = await client.query(`SELECT id, expires_at, verified
       FROM otp_verifications
       WHERE email = $1 AND otp_code = $2
       ORDER BY created_at DESC
       LIMIT 1`, [user.email.toLowerCase().trim(), otpCode]);
        if (otpRows.length === 0) {
            await client.query('ROLLBACK');
            return res.status(400).json({ message: 'Invalid OTP code' });
        }
...
router.post('/forgot-password/send-otp', async (req, res) => {
    ...
    try {
        ...
        await client.query('DELETE FROM otp_verifications WHERE email = $1 AND verified = FALSE', [email.toLowerCase().trim()]);
        // Insert new OTP
        await client.query(`INSERT INTO otp_verifications (email, otp_code, expires_at)
       VALUES ($1, $2, $3)`, [email.toLowerCase().trim(), otpCode, expiresAt]);
        // Send email via Resend
...
router.post('/forgot-password/reset', async (req, res) => {
    ...
    try {
       ...
       LIMIT 1`, [email.toLowerCase().trim(), otpCode]);
        ...
        // Prevent reusing the same password
        const { rows: userRows } = await client.query('SELECT password_hash FROM users WHERE email = $1', [email.toLowerCase().trim()]);
        if (userRows.length > 0) {
            const isSame = await bcrypt_1.default.compare(String(newPassword), userRows[0].password_hash);
            if (isSame) {
                await client.query('ROLLBACK');
                return res
                    .status(400)
       ...
       WHERE user_id = $1 AND revoked_at IS NULL`, [rows[0].id]);
        await client.query('COMMIT');
...
router.post('/reset-password', auth_1.authenticateToken, async (req, res) => {
    const { currentPassword, newPassword } = req.body ?? {};
    const userId = req.user.id;
    if (!currentPassword || !newPassword) {
        return res.status(400).json({
    ...
    try {
        ...
        // Verify current password
        const valid = await bcrypt_1.default.compare(String(currentPassword), rows[0].password_hash);
        if (!valid) {
            await client.query('ROLLBACK');
            return res.status(401).json({ message: 'Current password is incorrect' });
        }
        // Prevent reusing the same password
        const isSame = await bcrypt_1.default.compare(String(newPassword), rows[0].password_hash);
        if (isSame) {
            await client.query('ROLLBACK');
            return res
                .status(400)
